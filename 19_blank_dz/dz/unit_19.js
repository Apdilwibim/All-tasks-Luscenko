...
function t1() {
    ...
    document.querySelector('.out-1').textContent = a.textContent;
    return a;
}
...
function t3() {
    ...
    let b = document.querySelector('.div-3');
    w3 += 5;
    b.style.width = `${w3}px`;
    a.textContent = w3;
}
...
function t4() {
    ...
    let b = document.querySelector('.div-4');
...
let a = document.querySelector('.div-15').textContent;
...
function t19(event) {
    ...
    for (let i = 0; i < event.path[0].classList.length; i++) {
        result += event.path[0].classList[i] + ' ';
    }
    div19.textContent = result;
}
