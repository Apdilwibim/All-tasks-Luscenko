...
document.querySelector('.out-4').textContent = e1 + f1; //конкатенация
...
document.querySelector('.out-5').textContent = e2 / f2; //деление на ноль дает бесконечность
...
document.querySelector('.out-6').textContent = e3 + f3; //конкатенация
...
document.querySelector('.out-7').textContent = e4 * f4; //NaN
...
function t13() {
    out13.innerHTML = +i131.value + +i132.value;
}
...
function t16() {
    out16.innerHTML = +i161.value + +i162.value;
}
...
function t19() {
    out19.innerHTML = parseInt(i191.value) + parseInt(i192.value);
}
...
function t20() {
    out20.innerHTML = 'Уважаемый ' + firstName.value + ', ' + lastName.value + ', ' + 'ваш возраст ' + age.value + ', ' + 'по профессии вы ' + proff.value;
}
